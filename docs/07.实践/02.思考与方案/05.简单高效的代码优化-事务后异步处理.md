...
3. 新增`Redisson`组件，替换原本的基于lettuce手动编写的加解锁，将加锁代码写入`try catch`中，避免指令已发送到机器加上了锁，但加锁返回结果超时未被异常捕获，无法解锁，造成永不过期的锁。`Redisson`作为`Redis`官方指定的分布式`Redis`组件，无需担心许多分布式场景下的加解锁、续期、释放了非加锁线程的锁等问题，方案拉满，已经非常成熟。这里不做展开涉及。
